	...
	describe "Passes W3C tests", ->
		readdirSync("!TEST!/Turtle/TurtleTests/")
			...
			.map ( file ) => it "Passes \"#{ file.slice(0, -4) }\"", ->
				...
				switch
					when testManifest.a "http://www.w3.org/ns/rdftest#TestTurtlePositiveSyntax" then do (expect (=> Graph.fromTurtle.call { baseURI: testManifest.getPredicate "http://www.w3.org/2001/sw/DataAccess/tests/test-manifest#action" }, readFileSync "!TEST!/Turtle/TurtleTests/#{ file }"), "action").not.to.throw
					when testManifest.a "http://www.w3.org/ns/rdftest#TestTurtleNegativeSyntax" then do (expect (=> Graph.fromTurtle.call { baseURI: testManifest.getPredicate "http://www.w3.org/2001/sw/DataAccess/tests/test-manifest#action" }, readFileSync "!TEST!/Turtle/TurtleTests/#{ file }"), "action").to.throw
					...
					when testManifest.a "http://www.w3.org/ns/rdftest#TestTurtleNegativeEval" then do (expect (=> Graph.fromTurtle.call { baseURI: testManifest.getPredicate "http://www.w3.org/2001/sw/DataAccess/tests/test-manifest#action" }, readFileSync "!TEST!/Turtle/TurtleTests/#{ file }"), "action").to.throw
